...
-- (3) ORDER BY 절 : 정렬
--  5) emp 테이블에서 job 을 중복배제하여 조회하고 결과는 오름차순으로 정렬
SELECT DISTINCT
       e.JOB
  FROM emp e
 ORDER BY e.JOB
...
SELECT e.EMPNO
     , e.ENAME
  FROM emp e
 WHERE e.ENAME LIKE '_M%'  -- 유사 패턴 인식 문자 _를 사용하여 한글자로 제한
;  
...
SELECT e.EMPNO
     , e.ENAME
  FROM emp e
 WHERE e.ENAME LIKE '__M%'  -- 유사 패턴 인식 문자 _를 두번 사용하여 앞자리를 2글자로 제한
;  
